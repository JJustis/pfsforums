...
document.addEventListener('DOMContentLoaded', () => {
   ...
   // Add this to the App object
// Function to load user products with ownership filtering
async loadUserProducts() {
    ...
    try {
      ...
      let url = `product_api.php?action=list&username=${encodeURIComponent(username)}`;
if (token) {
    url += `&token=${encodeURIComponent(token)}`;
        ...
        // Proceed if success or if there's a products array anyway
        if (data.success || (data.products && Array.isArray(data.products))) {
            ...
            if (products.length === 0) {
                productsList.innerHTML = `
                    <div class="empty-message">
                        ...
                        <p>Use the form below to upload your first product.</p>
                    </div>
                `;
                return;
            }
            
            // Render products with maximum safeguards against missing data
            productsList.innerHTML = '';
            ...
            products.forEach(product => {
                ...
                const productId = product.id || `unknown_${Date.now()}`;
                const productName = product.name || 'Unnamed Product';
                const productDesc = product.description || 'No description available';
                const productPrice = typeof product.price === 'number' ? product.price : 9.99;
                const fileName = product.file_name || 'file.ext';
                const fileSize = product.file_size ? this.formatFileSize(product.file_size) : 'unknown size';
                const createdAt = product.created_at ? this.formatDate(product.created_at) : new Date().toLocaleString();
                const ownerUsername = product.owner_username || username;
                
                productCard.dataset.productId = productId;
                productCard.dataset.owner = ownerUsername;
                ...
                productCard.innerHTML = `
                    ...
                    <div class="product-card-body">
                        <p>${this.escapeHtml(productDesc)}</p>
                        <div class="product-meta">
                            <span class="product-price">$${productPrice.toFixed(2)}</span>
                            <span class="product-file">${this.escapeHtml(fileName)} (${fileSize})</span>
                            <span class="product-date">Uploaded: ${createdAt}</span>
                            <span class="product-owner"><strong>Owner: ${this.escapeHtml(ownerUsername)}</strong></span>
                        </div>
                    </div>
                    <div class="product-card-footer">
                        <div class="product-code">
                            <p>Use this code in your post to sell this product:</p>
                            <code class="product-markup">[paypal:product=${this.escapeHtml(productName)} price=${productPrice} email=${this.currentUser.email || 'your@email.com'} file=${productId} owner=${this.escapeHtml(ownerUsername)}]
${this.escapeHtml(productDesc)}
[/paypal]</code>
                            <button class="btn btn-secondary btn-sm copy-markup-btn">
                ...
                const copyBtn = productCard.querySelector('.copy-markup-btn');
                if (copyBtn) {
                    copyBtn.addEventListener('click', () => {
                        const markup = productCard.querySelector('.product-markup')?.textContent || '';
                        if (navigator.clipboard) {
                            navigator.clipboard.writeText(markup)
                                .then(() => this.showToast('success', 'Product markup copied to clipboard'))
                                .catch(err => this.showToast('error', 'Failed to copy: ' + err.message));
                        } else {
                            ...
                            try {
                                ...
                                this.showToast('success', 'Product markup copied to clipboard');
                            } catch (err) {
                                this.showToast('error', 'Failed to copy: ' + err.message);
                            }
            ...
            throw new Error(data.error || 'Could not load products data');
        }
    } catch (error) {
        ...
        productsList.innerHTML = `
            <div class="error-message">
                <p>We encountered an issue loading your products.</p>
                <p>Don't worry! This might be because:</p>
                <ul>
                    <li>You haven't created any products yet</li>
                    <li>The product system is still being set up</li>
                ...
                <div class="error-actions">
                    ...
                    <button class="btn btn-secondary" id="create-first-product">
                        <i class="fas fa-plus"></i> Create Your First Product
                    </button>
                </div>
                <div class="error-details" style="margin-top: 15px; font-size: 0.8em; color: #888;">
                    <details>
        ...
        document.getElementById('create-first-product')?.addEventListener('click', () => {
            ...
            const uploadForm = document.querySelector('.product-upload-form') || 
                               document.getElementById('product-upload-section');
...
async deleteProduct(productId, productName) {
    ...
    this.showModal(
        'Delete Product',
        `Are you sure you want to delete "${productName || 'this product'}"? This cannot be undone.`,
        'Delete',
        'Cancel',
        async () => {
            try {
                ...
                if (data.success) {
                    this.showToast('success', data.message || 'Product deleted successfully');
...
// Ultra-robust setupProductUpload function
setupProductUpload() {
    ...
    uploadBtn.addEventListener('click', async () => {
        ...
        // Validate inputs
        if (!nameInput || !descInput || !priceInput || !fileInput) {
            this.showToast('error', 'Could not find all form fields. Please refresh the page.');
            return;
        }
        
        // Check if all fields have values
        if (!nameInput.value || !descInput.value || !priceInput.value) {
            this.showToast('error', 'Please fill in all product details');
            return;
        }
        ...
        if (fileInput.files[0].size > maxSize) {
            this.showToast('error', `File too large. Maximum size is ${(maxSize / 1024 / 1024).toFixed(0)}MB`);
            return;
        }
        ...
        try {
            // Get user information (don't require authentication token)
            ...
            formData.append('productDescription', descInput.value);
            ...
            // Wrap XHR in a promise for easier handling
            ...
            // Start upload
            xhr.open('POST', 'upload_product.php');
            xhr.send(formData);
            ...
            if (response.success) {
                ...
                this.showToast('success', response.message || 'Product uploaded successfully');
            ...
            this.showToast('error', `Error uploading product: ${error.message}`);
        } finally {
            // Reset UI
            ...
            uploadBtn.innerHTML = '<i class="fas fa-cloud-upload-alt"></i> Upload Product';
        }
    });
},
// Format file size
...
// Setup profile page elements and event listeners
setupProfilePage() {
    ...
    // Signature saving functionality
    const saveSignatureBtn = document.getElementById('save-signature-btn');
    const signatureInput = document.getElementById('profile-signature');
    
    if (saveSignatureBtn && signatureInput) {
        saveSignatureBtn.addEventListener('click', () => {
            this.saveSignature();
        });
        
        // Add character counter for signature
        signatureInput.addEventListener('input', () => {
            const maxLength = 200; // Maximum signature length
            const remaining = maxLength - signatureInput.value.length;
            
            // Create or update counter element
            let counterEl = document.getElementById('signature-counter');
            if (!counterEl) {
                counterEl = document.createElement('div');
                counterEl.id = 'signature-counter';
                counterEl.className = 'character-counter';
                signatureInput.insertAdjacentElement('afterend', counterEl);
            }
    ...
    // Setup product upload system
    this.setupProductUpload();
    
    // Load user's products
    this.loadUserProducts();
    
    // Load user stats and activity data
    this.loadUserStatsForProfile();
    
    // Load user avatar
    this.loadUserAvatar();
    
    // Add tab navigation if profile has tabs
    ...
    if (profileTabs.length && profileTabContents.length) {
        profileTabs.forEach(tab => {
            tab.addEventListener('click', (e) => {
                ...
                profileTabContents.forEach(c => c.classList.add('hidden'));
    ...
    if (deleteProfileBtn) {
        deleteProfileBtn.addEventListener('click', (e) => {
            ...
            this.showModal(
                'Delete Profile',
                'Are you sure you want to delete your profile? This action cannot be undone and all your data will be permanently lost.',
                'Delete',
                'Cancel',
                () => {
